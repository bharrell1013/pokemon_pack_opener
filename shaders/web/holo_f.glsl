...
uniform sampler2D rainbowGradient; // Used for Iridescence
...
void main()
{
    ...
    if (cardRarity == 0) { // --- Normal Card ---
        ...
        if (!isInsideArtwork) {
            ...
            vec3 iridescentColor = texture(rainbowGradient, vec2(rainbowCoord, 0.5)).rgb;
        ...
        if (borderMask > 0.001) {
            ...
            vec3 addedIridescence = texture(rainbowGradient, vec2(borderRainbowCoord, 0.5)).rgb * borderIridescenceIntensity;
     ...
     if (renderMode != 0) {
         ...
         if (renderMode == 1) { // Iridescence Component Only
             ...
             if (cardRarity == 1 && !isInsideArtwork) {
                 float viewFactor = pow(dotNV, reverseIridescenceContrast);
                 float rainbowCoord = fract(viewFactor * reverseIridescenceFreq + time * 0.1);
                 vec3 reverseIrid = mix(texture(rainbowGradient, vec2(rainbowCoord, 0.5)).rgb, typeColor, 0.4) * reverseIridescenceIntensity;
                 baseIridContribution = reverseIrid * l_systemMask * reversePatternMaskInfluence * reverseEffectIntensity;
             } else if (cardRarity >= 2) {
                 float viewFactor = pow(dotNV, glossyIridescenceContrast);
                 float rainbowCoord = fract(viewFactor * glossyIridescenceFreq + time * 0.1);
                 vec3 glossyIrid = mix(texture(rainbowGradient, vec2(rainbowCoord, 0.5)).rgb, typeColor, 0.4) * glossyIridescenceIntensity;
             ...
             if (cardRarity >= 2) {
                 ...
                 if (bMask > 0.001) {
                     float borderRainbowCoord = fract(TexCoord.x * 1.8 - TexCoord.y * 0.7 + time * 0.2);
                     vec3 borderIridescenceOnly = texture(rainbowGradient, vec2(borderRainbowCoord, 0.5)).rgb * borderIridescenceIntensity * borderOverallIntensity;
